...
// ===== CHAT FILE UPLOAD =====
const uploadDir = path_1.default.join(publicPath, "uploads", "chat");
...
app.post("/api/register", async (req, res) => {
    try {
        const body = req.body;
        const result = await (0, db_1.query)(`INSERT INTO users (full_name, bhxh_code, cccd, phone, email, account_type, status,
...
app.post("/api/admin/login", async (req, res) => {
    ...
    if (username === ADMIN_USER && password === ADMIN_PASS) {
        ...
        await (0, db_1.query)("INSERT INTO admin_tokens (token) VALUES ($1)", [token]);
...
app.post("/api/admin/logout", async (req, res) => {
    ...
    if (token) {
        await (0, db_1.query)("DELETE FROM admin_tokens WHERE token = $1", [token]);
    }
...
app.get("/api/admin/check", async (req, res) => {
    ...
    const result = await (0, db_1.query)("SELECT id FROM admin_tokens WHERE token = $1", [token]);
...
app.get("/api/admin/users", async (_req, res) => {
    try {
        const result = await (0, db_1.query)(`SELECT id, full_name, bhxh_code, cccd, phone, email, account_type, status,
...
app.get("/api/admin/users/:id", async (req, res) => {
    try {
        const result = await (0, db_1.query)("SELECT * FROM users WHERE id = $1", [parseInt(req.params.id)]);
        if (result.rows.length === 0)
...
app.put("/api/admin/users/:id/status", async (req, res) => {
    try {
        const result = await (0, db_1.query)("UPDATE users SET status = $1 WHERE id = $2 RETURNING *", [req.body.status, parseInt(req.params.id)]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: "Not found" });
...
app.get("/api/admin/stats", async (_req, res) => {
    try {
        ...
        const approved = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE status = 'approved'");
        const pending = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE status = 'pending'");
        const rejected = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE status = 'rejected'");
        const individual = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE account_type = 'individual'");
        const org = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE account_type = 'organization'");
        res.json({
...
app.get("/api/admin/dashboard", async (_req, res) => {
    try {
        const totalUsers = await (0, db_1.query)("SELECT COUNT(*) as c FROM users");
        const activeUsers = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE status IN ('approved', 'pending')");
        const pendingApprovals = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE status = 'pending'");
        const todayReg = await (0, db_1.query)("SELECT COUNT(*) as c FROM users WHERE registered_at != '' AND registered_at::timestamp::date = CURRENT_DATE");
        const monthData = await (0, db_1.query)(`SELECT TO_CHAR(registered_at::timestamp, 'YYYY-MM') as month, COUNT(*) as count
      FROM users WHERE registered_at != '' AND registered_at IS NOT NULL
      GROUP BY month ORDER BY month LIMIT 6`);
        const labels = ["T1", "T2", "T3", "T4", "T5", "T6", "T7", "T8", "T9", "T10", "T11", "T12"];
        const registrationsByMonth = monthData.rows.map((r) => ({
            month: labels[parseInt(r.month.split("-")[1]) - 1] || r.month,
            count: parseInt(r.count),
        }));
        const recentUsers = await (0, db_1.query)(`SELECT full_name, status, registered_at FROM users
      ORDER BY registered_at DESC NULLS LAST LIMIT 7`);
        const recentActivity = recentUsers.rows.map((r) => {
            let time = "--:--";
...
app.get("/api/appointments", async (_req, res) => {
    try {
        const result = await (0, db_1.query)("SELECT * FROM appointments ORDER BY id");
...
app.post("/api/appointments", async (req, res) => {
    try {
        const body = req.body;
        const result = await (0, db_1.query)(`INSERT INTO appointments (full_name, phone, email, bhxh_code, date, time_slot, service, note)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`, [body.fullName || "", body.phone || "", body.email || "", body.bhxhCode || "",
...
app.put("/api/appointments/:id/status", async (req, res) => {
    try {
        const result = await (0, db_1.query)("UPDATE appointments SET status = $1 WHERE id = $2 RETURNING *", [req.body.status, parseInt(req.params.id)]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: "Not found" });
...
app.get("/api/chat/conversation/by-card/:cardId", async (req, res) => {
    try {
        const convResult = await (0, db_1.query)("SELECT * FROM chat_conversations WHERE id_card = $1 AND status = 'active' ORDER BY id DESC LIMIT 1", [req.params.cardId]);
        if (convResult.rows.length === 0) {
...
app.put("/api/chat/conversations/:id/status", adminAuth, async (req, res) => {
    try {
        await (0, db_1.query)("UPDATE chat_conversations SET status = $1 WHERE id = $2", [req.body.status || "closed", parseInt(req.params.id)]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: "Server error" });
...
app.get("/api/lookup/:code", async (req, res) => {
    try {
        ...
        if (!code)
            return res.status(400).json({ error: "Thiếu mã số BHXH" });
        const result = await (0, db_1.query)(`SELECT full_name, bhxh_code, cccd, phone, email, account_type, status,
        province, ward, street, gender, ethnicity, birth_date, registration_location
       FROM users WHERE bhxh_code = $1 LIMIT 1`, [code]);
        if (result.rows.length === 0) {
...
app.get("/tra-cuu", (req, res) => {
    res.sendFile(path_1.default.join(publicPath, "tracuu.html"));
});
// ===== START =====
